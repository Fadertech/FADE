...
{
    // Convert Fade:// to Fade:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Fade://"))
    {
        uri.replace(0, 12, "Fade:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Fade.lnk";
}
...
{
    return GetAutostartDir() / "Fade.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Fade-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Fade-qt [" + tr("command-line options") + "]                     " + "\n";
